...
export function WorkflowsPage() {
  ...
  useEffect(() => {
    if (selectedWorkflow) {
      api.get(`/workflows/${selectedWorkflow.id}/runs`).then((res) => setRuns(res.data));
    }
  }, [selectedWorkflow?.id]);
  ...
  const toggleWorkflow = async (workflow: Workflow) => {
    ...
    const updated = { ...workflow, enabled: res.data.enabled };
  ...
  const addAction = async (afterIndex: number, type: ActionType) => {
    ...
    setSelectedWorkflow(res.data);
    setWorkflows((prev) => prev.map((w) => (w.id === res.data.id ? res.data : w)));
