...
import { useCreateListInSpace, useCreateListInFolder } from '@/hooks/useLists';
...
export function CreateListModal({ open, onClose, spaceId, folderId }: CreateListModalProps) {
  ...
  const createInSpace = useCreateListInSpace(spaceId);
  const createInFolder = useCreateListInFolder(folderId);

  const mutation = folderId ? createInFolder : createInSpace;
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Create List">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              className="w-full h-9 px-3 rounded-lg bg-bg-tertiary border border-border-secondary text-sm text-text-primary transition-all duration-150 ease-out hover:border-border-primary focus:border-accent-blue focus:ring-1 focus:ring-accent-blue/30 focus:outline-none"
            />
          </div>
          <div className="flex flex-col gap-1.5">
