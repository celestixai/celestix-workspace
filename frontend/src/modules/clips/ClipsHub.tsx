      ...
      <div className="shrink-0 px-6 py-4 border-b border-border">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold text-text-primary">Clips</h1>
          </div>
        ...
        {stats && (
          <div className="flex items-center gap-6 mb-4">
            <div className="flex items-center gap-2 text-sm text-text-secondary">
              ...
              <span className="font-medium text-text-primary">{stats.totalClips}</span> clips
            </div>
            <div className="flex items-center gap-2 text-sm text-text-secondary">
              <Clock className="w-4 h-4" />
              <span className="font-medium text-text-primary">{formatTotalDuration(stats.totalDuration)}</span> total
            </div>
            <div className="flex items-center gap-2 text-sm text-text-secondary">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 bg-bg-secondary border border-border rounded-lg text-sm text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-accent-blue"
            />
          </div>
        </div>
      </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="aspect-video bg-bg-secondary rounded-xl animate-pulse" />
            ))}
          </div>
        ) : filtered.length === 0 ? (
