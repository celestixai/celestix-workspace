...
export function ChatView({ chatId }: ChatViewProps) {
  ...
  const { data: searchUsers } = useQuery<Array<{ id: string; displayName: string; email: string; avatarUrl?: string | null }>>({
    ...
    enabled: showAddMembers && addMemberSearch.length >= 1,
  });

  const addMemberMutation = useMutation({
    mutationFn: async (userId: string) => {
      await api.post(`/messenger/chats/${chatId}/members`, { userIds: [userId] });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['messenger', 'chat', chatId] });
  ...
  const removeMemberMutation = useMutation({
    mutationFn: async (targetUserId: string) => {
      await api.delete(`/messenger/chats/${chatId}/members/${targetUserId}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['messenger', 'chat', chatId] });
  ...
  const leaveMutation = useMutation({
    mutationFn: async () => {
      await api.post(`/messenger/chats/${chatId}/leave`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['messenger', 'chats'] });
      toast('Left the chat', 'success');
      setGroupInfoOpen(false);
    },
    onError: () => toast('Failed to leave', 'error'),
  });

  const inviteLinkMutation = useMutation({
    mutationFn: async () => {
      const { data } = await api.post(`/messenger/chats/${chatId}/invite-link`);
      return data.data.inviteLink as string;
    },
    onSuccess: (link) => {
      setInviteLink(link);
    },
    onError: () => toast('Failed to generate invite link', 'error'),
  });
      ...
      {groupInfoOpen && chat && chat.type !== 'DM' && (
        <div className="w-[320px] flex-shrink-0 border-l border-border-primary bg-bg-secondary flex flex-col h-full overflow-y-auto animate-slide-in-right">
          <div className="flex items-center justify-between px-4 h-14 border-b border-border-primary flex-shrink-0">
            ...
            <button onClick={() => { setGroupInfoOpen(false); setShowAddMembers(false); setInviteLink(null); }} className="p-1.5 rounded-lg hover:bg-bg-hover text-text-tertiary hover:text-text-primary transition-colors">
              <X size={18} />
            </button>
          </div>
          <div className="flex flex-col items-center py-6 px-4 gap-2">
            {chat.type === 'CHANNEL' ? (
              <div className="h-16 w-16 rounded-full bg-accent-blue/10 flex items-center justify-center">
                <Hash size={32} className="text-accent-blue" />
              </div>
            ) : (
          ...
          {/* Invite link section (channels) */}
          {chat.type === 'CHANNEL' && (
            <div className="px-4 mb-4">
              {inviteLink ? (
                <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-bg-tertiary border border-border-secondary">
                  <Link2 size={14} className="text-accent-blue flex-shrink-0" />
                  <span className="text-xs text-text-secondary truncate flex-1">{inviteLink}</span>
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(inviteLink);
                      toast('Invite link copied', 'success');
                    }}
                    className="p-1 rounded hover:bg-bg-hover text-text-tertiary hover:text-text-primary transition-colors flex-shrink-0"
                  >
                    <Copy size={14} />
                  </button>
                </div>
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-accent-blue bg-accent-blue/5 hover:bg-accent-blue/10 border border-accent-blue/10 transition-colors"
                >
                  <Link2 size={16} />
            ...
            <div className="px-4 mb-3">
              ...
              {searchUsers && searchUsers.length > 0 && (
                <div className="mt-2 max-h-[180px] overflow-y-auto space-y-0.5">
                  {searchUsers
                    .filter((u) => !chat.members?.some((m) => m.id === u.id))
                    .map((user) => (
                      <button
                        key={user.id}
                        onClick={() => {
              ...
              {addMemberSearch.length >= 1 && searchUsers?.filter((u) => !chat.members?.some((m) => m.id === u.id)).length === 0 && (
                <p className="text-xs text-text-tertiary text-center py-3">No users found</p>
              )}
            </div>
          )}
          ...
          <div className="px-4">
            ...
            <div className="space-y-0.5">
              {chat.members?.map((member) => (
                <div
                  key={member.id}
                  className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-bg-hover transition-colors group/member"
                >
                  <button
                    onClick={() => {
                      setGroupInfoOpen(false);
                  ...
                  {member.id !== currentUser?.id && (
                    <button
                      ...
                      className="p-1 rounded hover:bg-accent-red/10 text-text-tertiary hover:text-accent-red opacity-0 group-hover/member:opacity-100 transition-all flex-shrink-0"
                      title="Remove member"
                    >
                      <X size={14} />
                    </button>
          ...
          {/* Actions */}
          <div className="px-4 mt-4 space-y-1 pb-4">
            <p className="text-xs font-semibold text-text-tertiary uppercase tracking-wider mb-2">Actions</p>
            <button
              onClick={() => { setShowAddMembers(!showAddMembers); setAddMemberSearch(''); }}
              className={cn(
            ...
            {chat.type === 'CHANNEL' && !inviteLink && (
              <button
                onClick={() => inviteLinkMutation.mutate()}
                className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-text-secondary hover:bg-bg-hover hover:text-text-primary transition-colors"
              >
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-accent-red hover:bg-accent-red/10 transition-colors"
            >
