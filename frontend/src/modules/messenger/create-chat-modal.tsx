...
import { Search, X, Users, Hash, MessageSquare, ArrowRight, Check } from 'lucide-react';
...
export function CreateChatModal({ open, onClose, onChatCreated }: CreateChatModalProps) {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      if (chatType !== 'DIRECT') {
        payload.name = groupName || selectedUsers.map((u) => u.displayName.split(' ')[0]).join(', ');
    ...
    <Modal open={open} onClose={handleClose} title="New Conversation" size="md">
      ...
      {step === 'details' && (
        <div className="space-y-4">
          ...
          {/* Actions */}
          <div className="flex items-center justify-between pt-2 border-t border-border-primary">
            <Button variant="ghost" size="sm" onClick={() => setStep('users')}>
              Back
            </Button>
            <Button
              size="sm"
