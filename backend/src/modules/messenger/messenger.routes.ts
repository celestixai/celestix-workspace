...
router.post('/polls/:pollId/close', authenticate, async (req, res, next) => {
  try {
    const existing = await prisma.poll.findUnique({ where: { id: req.params.pollId } });
    if (!existing) return res.status(404).json({ success: false, error: 'Poll not found' });
    if (existing.creatorId !== req.user!.id) return res.status(403).json({ success: false, error: 'Not authorized' });
...
router.delete('/saved/:id', authenticate, async (req, res, next) => {
  try {
    const existing = await prisma.savedMessage.findUnique({ where: { id: req.params.id } });
    if (!existing || existing.userId !== req.user!.id) return res.status(404).json({ success: false, error: 'Saved message not found' });
    await prisma.savedMessage.delete({ where: { id: req.params.id } });
...
router.delete('/scheduled/:id', authenticate, async (req, res, next) => {
  try {
    ...
    if (!existing || existing.userId !== req.user!.id) return res.status(404).json({ success: false, error: 'Scheduled message not found' });
...
router.patch('/folders/:id', authenticate, async (req, res, next) => {
  try {
    const existing = await prisma.chatFolder.findUnique({ where: { id: req.params.id } });
    if (!existing || existing.userId !== req.user!.id) return res.status(404).json({ success: false, error: 'Folder not found' });
...
router.delete('/folders/:id', authenticate, async (req, res, next) => {
  try {
    const existing = await prisma.chatFolder.findUnique({ where: { id: req.params.id } });
    if (!existing || existing.userId !== req.user!.id) return res.status(404).json({ success: false, error: 'Folder not found' });
    await prisma.chatFolder.delete({ where: { id: req.params.id } });
    res.json({ success: true });
  } catch (err) { next(err); }
});
