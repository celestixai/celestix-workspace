...
export class DashboardService {
  ...
  async getActivityFeed(userId: string, limit: number) {
    ...
    const [recentMessages, recentEmails, recentTasks, recentNotes, recentFormResponses, recentAppointments, recentVideos] =
      await Promise.all([
        ...
        prisma.appointment.findMany({
          where: {
            OR: [
              { service: { bookingPage: { userId } } },
              { guestEmail: { not: '' } },
            ],
            startAt: { gte: new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000) },
          },
          orderBy: { startAt: 'desc' },
          take: 5,
          include: { service: { select: { name: true, bookingPageId: true } } },
        }),
    ...
    for (const a of recentAppointments) {
      items.push({ type: 'appointment', module: 'bookings', id: a.id, title: `Booking: ${a.service.name}`, preview: a.guestName || undefined, timestamp: a.startAt, link: `/bookings/${a.service.bookingPageId}` });
    }
  ...
  async getStats(userId: string) {
    const [
      ...
      prisma.appointment.count({ where: { service: { bookingPage: { userId } }, status: 'CONFIRMED' } }),
