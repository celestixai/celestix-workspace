...
export class WorkspaceService {
  ...
  async getWorkspaces(userId: string) {
    ...
    return workspaces.map((ws) => ({
      ...ws,
      myRole: ws.members[0]?.role || 'MEMBER',
      members: undefined,
    }));
  }
  ...
  async createInvite(userId: string, workspaceId: string, input: CreateInviteInput) {
    ...
    if (!membership || (membership.role !== 'OWNER' && membership.role !== 'ADMIN')) {
      throw new AppError(403, 'Only workspace owners and admins can create invites', 'FORBIDDEN');
    }

    const token = crypto.randomBytes(9).toString('base64url').substring(0, 12);

    const expiresInMs: Record<string, number | null> = {
      '1h': 60 * 60 * 1000,
      '12h': 12 * 60 * 60 * 1000,
      '1d': 24 * 60 * 60 * 1000,
      '7d': 7 * 24 * 60 * 60 * 1000,
    ...
    const ms = expiresInMs[input.expiresIn ?? '7d'];
    const expiresAt = ms != null ? new Date(Date.now() + ms) : null;
    const maxUses = maxUsesMap[input.maxUses ?? 'unlimited'];
  ...
  async getInvites(userId: string, workspaceId: string) {
    ...
    if (!membership || (membership.role !== 'OWNER' && membership.role !== 'ADMIN')) {
      throw new AppError(403, 'Only workspace owners and admins can view invites', 'FORBIDDEN');
    }
  ...
  async revokeInvite(userId: string, workspaceId: string, inviteId: string) {
    ...
    if (!membership || (membership.role !== 'OWNER' && membership.role !== 'ADMIN')) {
      throw new AppError(403, 'Only workspace owners and admins can revoke invites', 'FORBIDDEN');
    }
  ...
  async validateInviteToken(token: string) {
    const invite = await prisma.workspaceInvite.findUnique({
      ...
      include: {
        workspace: {
          select: { id: true, name: true, slug: true, iconUrl: true, _count: { select: { members: true } } },
        },
      },
    });

    if (!invite) {
      throw new AppError(404, 'Invalid invite link', 'NOT_FOUND');
    }
    if (invite.revoked) {
      throw new AppError(410, 'This invite has been revoked', 'INVITE_REVOKED');
    }
    if (invite.expiresAt && invite.expiresAt < new Date()) {
      throw new AppError(410, 'This invite has expired', 'INVITE_EXPIRED');
    }
    if (invite.maxUses != null && invite.useCount >= invite.maxUses) {
      throw new AppError(410, 'This invite has reached its maximum number of uses', 'INVITE_MAX_USES');
    }
  ...
  async joinViaToken(userId: string, token: string) {
    const invite = await prisma.workspaceInvite.findUnique({
      where: { token },
      include: { workspace: true },
    });
    ...
    if (existing) {
      return { alreadyMember: true, workspace: invite.workspace };
    }

    // Add user as MEMBER and increment useCount in a transaction
    const [member] = await prisma.$transaction([
      prisma.workspaceMember.create({
        data: { workspaceId: invite.workspaceId, userId, role: 'MEMBER' },
        include: { workspace: true },
      }),
    ...
    return { alreadyMember: false, workspace: member.workspace };
  }
}
